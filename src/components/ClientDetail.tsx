...
import { 
  ...
  Smartphone, Copy, Check, Download
} from 'lucide-react';
...
export function ClientDetail({ client, onClose, profile }: ClientDetailProps) {
  ...
  const creditApiUrl = import.meta.env.VITE_CREDIT_API_URL || "https://gnsis-api.jamuywasi.com";
  ...
  const fetchCreditsFromApi = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || "Error al consultar el sistema de créditos.");
      }
      ...
      setApiError(error.message || "Error al consultar el sistema de créditos.");
    } finally {
      setIsFetching(false);
    }
  };
  ...
  const handleSimulatorDataConsult = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || "Error al consultar el simulador.");
      }
  ...
  return (
    <div className="bg-surface flex-1 flex flex-col min-h-0 h-full overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6 md:space-y-8 scrollbar-thin scrollbar-thumb-slate-800">
        ...
        {!externalCredits && !detailedCredit && (
          <div className="space-y-4">
            <button 
            ...
            >
              {isFetching ? (
                ...
                <>
                  <Search size={20} />
                  {apiError ? 'Reintentar Consulta en Sistema' : 'Consultar Créditos en Sistema'}
                </>
              )}
            </button>
        ...
        {externalCredits && externalCredits.length > 0 && !detailedCredit && (
          <section className="space-y-4">
            <div className="flex items-center justify-between gap-4 text-white mb-2 flex-wrap">
              <div className="flex items-center gap-2">
                <span className="text-xl">📊</span>
                <h4 className="text-sm font-black uppercase tracking-widest text-slate-200">Total créditos: {externalCredits.length}</h4>
              </div>
              <a 
                href={`${creditApiUrl}/api/credito/${client.dni}/pdf`}
                target="_blank" 
                rel="noopener noreferrer" 
                className="text-[9px] md:text-[10px] font-black bg-blue-500 hover:bg-blue-400 text-black px-3 py-1.5 rounded-xl uppercase tracking-widest flex items-center gap-1.5 transition-all cursor-pointer shadow-lg shadow-blue-550/20"
              >
            ...
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="flex gap-2 flex-wrap items-center">
                <a 
                  ...
                  target="_blank" 
                  rel="noopener noreferrer" 
                  className="text-[10px] font-black bg-blue-500 hover:bg-blue-400 text-black px-3 py-1.5 rounded-lg uppercase tracking-widest flex items-center gap-1.5 transition-all shadow-lg shadow-blue-500/10 cursor-pointer"
                >
                ...
                <div className="flex items-center justify-between px-2">
                  ...
                  <div className="flex flex-wrap gap-2 sm:gap-3">
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-[8px] font-bold text-slate-500 uppercase">Cobrada (Azul)</span>
                    </div>
                    <div className="flex items-center gap-1">
                      <div className="w-1.5 h-1.5 rounded-full bg-slate-500" />
                      <span className="text-[8px] font-bold text-slate-500 uppercase">No Cobrada (Gris)</span>
                    </div>
                    <div className="flex items-center gap-1">
                      <div className="w-1.5 h-1.5 rounded-full bg-emerald-500" />
                      <span className="text-[8px] font-bold text-slate-500 uppercase">Faltante (Verde)</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                  {Object.entries(detailedCredit.timeline).map(([month, data]: [string, any], idx) => {
                    const getStatusStyle = (status: string) => {
                      switch (status) {
                        case '🔵': return 'bg-blue-500/10 border-blue-500/20 text-blue-400';
                        case '🟢': return 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400';
                        case '⚪': return 'bg-slate-800/40 border-slate-800/80 text-slate-500 hover:border-slate-700';
                        default: return 'bg-slate-800/30 border-slate-800 text-slate-500';
                      }
                    };
                    const style = getStatusStyle(data.status);
                    const isUnpaid = data.status === '⚪';
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <span className={cn(
                            ...
                            isUnpaid && "line-through text-slate-600"
                          )}>{month}</span>
                          <span className="text-xs">{data.status}</span>
                        </div>
                        <div className={cn(
                          "text-[11px] font-mono font-bold",
                          isUnpaid ? "line-through text-slate-600 font-medium" : ""
                        )}>
