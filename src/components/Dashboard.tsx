...
export function Dashboard({ profile, externalSelectedClient }: { profile: UserProfile, externalSelectedClient?: Client | null }) {
  ...
  const fetchApiLeads = useCallback(async (currentOffset: number, currentLimit: number, currentSearch = "") => {
    setLoadingLeads(true);
    try {
      const response = await fetch(`/api/ugel/data?limit=${currentLimit}&offset=${currentOffset}&search=${encodeURIComponent(currentSearch)}`);
  ...
  useEffect(() => {
    if (view === 'leads') {
      fetchApiLeads(offset, pageSize, searchQuery);
    }
  ...
  const handleSearch = async (query: string) => {
    ...
    // Quick fallback: If it is exactly 8 digits numeric DNI, query the specific DNI endpoint 
    // to guarantee we fetch them even if they aren't in the cached broad list.
    if (query.length === 8 && /^\d+$/.test(query)) {
      ...
      try {
        ...
        // If not in API, check local dbClients as fallback
        ...
        console.error('Search DNI fallback error:', error);
      } finally {
